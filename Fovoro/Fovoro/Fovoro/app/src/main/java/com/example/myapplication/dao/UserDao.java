...
import com.example.myapplication.util.JDBCUtils;
...
public class UserDao {
    ...
    private static final String TAG = "mysql-as_userList2-UserDao";
    ...
    public int login(String userAccount, String userPassword){
        ...
        try {
            ...
            String sql = "select * from as_userList2 where userAccount = ?";
    ...
    public boolean register(User user){
        ...
        try {
            String sql = "insert into as_userList2(userAccount,userPassword,userName,userType,userState,userDel) values (?,?,?,?,?,?)";
            if (connection != null){// connection不为null表示与数据库建立了连接
                PreparedStatement ps = connection.prepareStatement(sql);
                if (ps != null){
